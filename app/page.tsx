...
function Flow({ turn }: { turn: ConversationTurn | null }) {
  ...
  return <section className="min-h-[760px] overflow-hidden rounded-3xl border border-emerald-200 bg-white/95 shadow-panel"><header className="border-b border-emerald-100 bg-emerald-50/60 px-5 py-4"><p className="text-xs font-bold text-emerald-700">Task-level orchestrator</p><h2 className="text-lg font-bold">SPN Shopping Agent</h2><p className="mt-1 text-sm text-emerald-900">Understanding과 Policy가 전체 흐름을 제어하고, RA-Rec은 상태·질의·추천을 담당합니다.</p></header><div className="max-h-[calc(100vh-170px)] space-y-2 overflow-auto p-5 scrollbar-thin">
...
function MemoryPanel({ turn, state }: { turn: ConversationTurn | null; state: DialogueState }) {
  ...
  return <aside className="min-h-[760px] overflow-hidden rounded-3xl border border-slate-200 bg-white/95 shadow-panel"><header className="border-b border-slate-100 px-5 py-4"><p className="text-xs font-bold text-slate-400">State, evidence, score</p><h2 className="text-lg font-bold">RA-Rec Dialogue State</h2></header><div className="max-h-[calc(100vh-170px)] space-y-4 overflow-auto p-5 scrollbar-thin">
    ...
    <Card className="border-blue-200"><CardHeader className="pb-2"><CardTitle className="flex items-center gap-2 text-sm"><Search className="h-4 w-4 text-blue-600" /> 검색 질의 / 상태</CardTitle></CardHeader><CardContent><p className="mb-2 rounded bg-blue-50 p-2 text-xs text-blue-900">{turn?.query ?? "Policy가 Ask User를 선택하면 생성하지 않습니다."}</p><pre className="max-h-40 overflow-auto rounded bg-slate-950 p-3 text-[10px] leading-4 text-slate-100">{JSON.stringify(state, null, 2)}</pre></CardContent></Card>
