...
import { Bot, CheckCircle2, CircleHelp, ClipboardList, Send, Star, UserRound } from "lucide-react";
...
function suggestionsFor(turn: ConversationTurn | null) {
  if (!turn) return ["태블릿을 처음 사는데 뭘 보고 골라야 할지 모르겠어요. 20만 원 이하였으면 좋겠어요.", "20만 원 이하 태블릿을 찾고 있어요.", "대학 수업 필기용 태블릿을 추천해줘."];
  if (turn.policy.action === "ask_user") return ["주로 학교에서 필기할 것 같아요.", "영상 시청과 웹서핑 위주로 사용할 것 같아요.", "후기가 많은 제품이 좋아요."];
  return ["첫 번째 제품을 자세히 볼게요.", "첫 번째 제품은 저장 공간이 너무 작아요.", "첫 번째와 두 번째를 비교해줘.", "이 제품으로 살게요."];
}
...
function Flow({ turn }: { turn: ConversationTurn | null }) {
  ...
  const steps = [
    { title: "사용자 이해", owner: "SPN", tone: "emerald" },
    { title: "상태 갱신", owner: "RA-Rec", tone: "blue" },
    { title: "행동 결정", owner: "SPN", tone: "emerald" },
    { title: "정보 탐색", owner: "SPN", tone: "emerald" },
    { title: "추천 생성", owner: "RA-Rec", tone: "blue" },
    { title: "응답 구성", owner: "SPN", tone: "emerald" }
  ] as const;
...
export default function Home() {
  ...
  const suggestions = suggestionsFor(currentTurn);
